...
int main()
{
// создаю окно размером 600на600 и 60 фпс
sf::RenderWindow window(sf::VideoMode(600, 600), "Project");
    ...
    sf::Event event;
	int move_counter = 0; // Счетчик случайных ходов
	...
	while (window.isOpen())
	{
		while (window.pollEvent(event)) {
			...
			if (event.type == sf::Event::KeyPressed) {
				...
				// Получаю нажатую клавишу - выполняю соответствующее действие
		...
		// Выполняю необходимиые действия по отрисовке
		window.clear();
		window.draw(game);
		window.draw(text);
		window.display();
