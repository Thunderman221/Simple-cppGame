#include "Game.h"
Game::Game() {
	// Подгружаю шрифт для отрисовки элементов
	font.loadFromFile("calibri.ttf");
	Init();
}

void Game::Init() {
	// Заполняю массив плашек
...
bool Game::Check() {
	//Проверка собраности головоломки
...
void Game::draw(sf::RenderTarget& target, sf::RenderStates states) const 
{
	...
	//Подготовливаю рамку для отрисовки всех плашек
	shape.setSize(sf::Vector2f(CELL_SIZE - 2, CELL_SIZE - 2));
	shape.setOutlineThickness(2.f);
	shape.setOutlineColor(color);
	shape.setFillColor(sf::Color::Transparent);

	// Подготавливаю текстовую заготовку для отрисовки номеров плашек
	...
	for (unsigned int i = 0; i < ARRAY_SIZE; i++) {
		...
		// Ресую все плашки, кроме пустой
		if (elements[i] > 0) {
			//Вычисление позиции плащки для отрисовки
			sf::Vector2f position(i % SIZE * CELL_SIZE + 10.f, i / SIZE * CELL_SIZE + 10.f);
			shape.setPosition(position);
			// Позицию текста подбирал в ручную
